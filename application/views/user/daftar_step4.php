<form method="post" action="<?= base_url() ?>main_user/save_alamat" enctype="multipart/form-data" id="form">
    ...
    <a href="<?= base_url('main_user/daftar?act=step3') ?>" class="btn btn-primary" type="button">Sebelumnya</a>
    <button type="submit" class="btn btn-success" type="button" <?php if (!empty($biodata)) {
                                                                                        if ($biodata['syarat2'] == 'Sudah') echo "disabled";
                                                                                    } ?>>Lanjut ke Data Orang Tua <i class="fa fa-chevron-right" aria-hidden="true"></i></button>
</form>